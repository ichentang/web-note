...
<script>
    ...
    document.querySelector('.header .time').innerHTML = `
      当前时间：${year}年${month}月${day}日-${h}时${m}分${s}秒
    `;
  },1000)
 </script>
~~~
...
需求1： 更换图例组件文字颜色 rgba(255,255,255,.5)  文字大小为12  以及 提示框组件修改。
...
- 修改水平居中 垂直居中
- 修改内圆半径和外圆半径为    ["40%", "60%"]    老师友情提示，带有直角坐标系的比如折线图柱状图是 grid修改图形大小，而我们饼形图是通过 radius 修改大小
...
注意：官方社区地址不能用了，需要自己去百度

> [社区]([Make A Pie](https://www.makeapie.com/explore.html)就是一些，活跃的echart使用者，交流和贡献定制好的图表的地方。
...
参考社区的例子：[Make A Pie - 模拟航线](https://www.makeapie.com/editor.html?c=x0-ExSkZDM)  (模拟飞机航线)
